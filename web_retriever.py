...
# https://github.com/langchain-ai/langchain/blob/master/libs/langchain/langchain/retrievers/web_research.py
...
class WebResearchRetriever(BaseRetriever):
    ...
    search: GoogleSearchAPIWrapper = Field(
        default=GoogleSearchAPIWrapper(), description="Google Search API Wrapper"
    )
...
"""async def _get_relevant_documents(
    ...
    *,
    ...
    # Get search questions
    logger.info("Generating questions for Google Search ...")
    result = await self.llm_chain({"question": query})
    logger.info(f"Questions for Google Search (raw): {result}")
    questions = getattr(result["text"], "lines", [])
    logger.info(f"Questions for Google Search: {questions}")
